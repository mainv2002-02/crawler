...
from excel_base import (
    ...
    StringIO,
    as_csv,
    as_dict_row_merge_xls,
    as_list_row_merge_xls,
    as_row_merge_xls,
    as_xls,
    is_py2,
)
...
def __init__(
        ...
        for sheet_name, sheet_info in self.data.items():
            ...
            if sheet_data:
                if isinstance(sheet_data[0], dict):
                    ...
                    sheet_data = [
                        [row[col] for col in sheet_headers] for row in sheet_data
                    ]
    ...
    if merge_type == "row_merge":
        _, content_type, file_ext = (
            self.as_row_merge_xls,
        ...
        use_xls = (
            True
            if len(self.data) <= self.EXCEL_MAXIMUM_ALLOWED_ROWS and not self.force_csv
            else False
        )
        _, content_type, file_ext = (
            (self.as_xls, "application/vnd.ms-excel", "xls")
            if use_xls
            else (self.as_csv, "text/csv", "csv")
        )
    self.output.seek(0)
    super(ExcelResponse, self).__init__(self.output, content_type=content_type)
    file_name_ext = (
        "_{0}".format(datetime.datetime.now().strftime(self.format))
        if self.format
        else ""
    )
    self["Content-Disposition"] = 'attachment;filename="%s.%s"' % (
        "{0}{1}".format(self.output_name, file_name_ext).replace('"', '"'),
        file_ext,
    )
...
if "FileResponse" in names:
    ...
    ExcelResponse = type(
        "StreamingHttpResponse", (http.StreamingHttpResponse,), clsdict
    )
else:
