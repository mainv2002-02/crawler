...
def get_merged_rows_num(data):
    return max(
        sum(get_merged_rows_num(d) if isinstance(d, list) else 0 for d in data), 1
    )
...
def row_preprocessing(data, mapping):
    ...
    return (
        field_value + [data_preprocessing(data.get(data_key), nextmapping)]
        if nextmapping
        else field_value
    )
...
def auto_adjust_width(self, sheet, colx, value, widths):
    ...
    if not self.auto_adjust_width:
        return
    width = (
        screen.calc_width(value) * 256
        if isinstance(value, basestring)
        else screen.calc_width(str(value)) * 256
    )
        ...
        for row in sheet_data:
            ...
            for colx, value in enumerate(row):
                if isinstance(value, list):
                    ...
                    value, cell_style = get_cell_info(self, value, cell_styles)
                    sheet.write_merge(
                        rowx, rowx + rowmax - 1, colx, colx, value, style=cell_style
                    )
                    auto_adjust_width(self, sheet, colx, value, widths)
...
def as_dict_row_merge_xls(self):
    ...
    self.data = {
        sheet_name: generate_sheet_info(sheet_info)
        for sheet_name, sheet_info in self.data.items()
    }
