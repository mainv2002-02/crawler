...
class TimeConvertTools(object):
    ...
    def value_format(
        self, value: Optional[str] = None, format: Optional[str] = None
    ) -> str:
        if format:
            return format
        return (
            self.LEN_FORMAT.get(len(value))
    ...
    def __utc_datetime(
        ...
        return self.several_time_coming(
            dt=utc_dt or self.basic_utc_datetime(),
            utc=True,
            timezone=timezone,
            years=years,
            months=months,
    ...
    def __local_datetime(
        ...
        return self.several_time_coming(
            dt=local_dt or self.basic_local_datetime(),
            utc=False,
            timezone=timezone,
            years=years,
            months=months,
    ...
    def __datetime(
        ...
        return dt or (
            self.basic_utc_datetime()
            if utc
            else self.basic_local_datetime(timezone=timezone)
        )
    ...
    def __relativedelta(
        ...
        return relativedelta(years=years, months=months) + datetime.timedelta(
            days=days,
            seconds=seconds,
            microseconds=microseconds,
            milliseconds=milliseconds,
    ...
    def __remove_ms_or_not(
        self, dt: datetime.datetime, ms: Optional[bool] = True
    ) -> datetime.datetime:
    ...
    def basic_utc_datetime(self, ms: bool = True) -> datetime.datetime:
        return self.__remove_ms_or_not(
            datetime.datetime.utcnow().replace(tzinfo=tz.UTC), ms=ms
        )

    def basic_local_datetime(
        self, ms: bool = True, timezone: Optional[str] = None
    ) -> datetime.datetime:
        # In[1]: import time
        #
        # In[2]: time.localtime()
        ...
        return self.__remove_ms_or_not(
            self.__to_local_datetime(
                self.basic_utc_datetime(), self.timezone(timezone)
            ),
            ms=ms,
        )
    ...
    def is_local_datetime(
        self, dt: datetime.datetime, local_tz: Optional[str] = None
    ) -> bool:
        """
        ...
        return str(dt.tzinfo) == str(
            None if local_tz == -1 else tz.gettz(self.timezone(local_tz))
        )

    def date_to_datetime(self, dt: datetime.date) -> datetime.datetime:
        return datetime.datetime(dt.year, dt.month, dt.day)

    def __to_utc_datetime(
        self, dt: datetime.datetime, timezone: Optional[str] = None
    ) -> datetime.datetime:
    ...
    def to_datetime(
        ...
        return dt + self.__relativedelta(
            years=years,
            months=months,
            days=days or idx,
            seconds=seconds,
    ...
    def yesterday_local_datetime(
        ...
        return self.__remove_ms_or_not(
            self.several_days_ago(utc=False, timezone=timezone, days=1), ms=ms
        )
    ...
    def tomorrow_local_datetime(
        ...
        return self.__remove_ms_or_not(
            self.several_days_coming(utc=False, timezone=timezone, days=1), ms=ms
        )

    def several_days_ago(
        self,
        dt: Optional[datetime.datetime] = None,
        utc: bool = True,
        ms: bool = True,
        ...
        return self.__remove_ms_or_not(
            self.__datetime(dt, utc, timezone=timezone) - datetime.timedelta(days=days),
            ms=ms,
        )

    def several_days_coming(
        self,
        dt: Optional[datetime.datetime] = None,
        utc: bool = True,
        ms: bool = True,
    ...
    def several_time_ago(
        ...
        return self.__remove_ms_or_not(
            self.__datetime(dt, utc, timezone=timezone)
            - self.__relativedelta(
                years=years,
                months=months,
                days=days,
                seconds=seconds,
    ...
    def to_date(
        ...
        if isinstance(value, datetime.datetime):
            ...
            return None
        return date + self.__relativedelta(
            years=years, months=months, days=days or idx, weeks=weeks
        )
    ...
    def is_the_same_day(
        ...
        return self.to_date(value1, format=format1 or format) == self.to_date(
            value2, format=format2 or format
        )
    ...
    def utc_yearweek(
        self,
        value: Union[datetime.datetime, datetime.date, None] = None,
        utc: bool = True,
        ms: bool = True,
        timezone: Optional[str] = None,
        years: int = 0,
        ...
        if yearweek_format != self.YEARWEEK_FORMAT_ISO:
            return Week.fromstring(
                self.utc_string(
                    value=value,
                    format=yearweek_format,
                    utc=utc,
                    ms=ms,
                    timezone=timezone,
                    years=years,
    ...
    def to_week(
        self, value: TimeAnyT, idx: int = 0, mode: int = 3, format: Optional[str] = None
    ) -> Optional[ISOWeek]:
    ...
    def to_isoweek(
        self, value: TimeAnyT, idx: int = 0, format: Optional[str] = None
    ) -> Optional[ISOWeek]:
    ...
    # DATETIME_STRING
    def datetime_to_unicode_string(
        self, dt: datetime.datetime, format: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            return (
                dt.strftime(format.encode("unicode-escape").decode())
                .encode()
                .decode("unicode-escape")
            )

    def datetime_to_string(
        self, dt: datetime.datetime, format: Optional[str] = None, isuc: bool = False
    ) -> str:
        if isuc:
            return self.datetime_to_unicode_string(dt, format=format)
        return dt.strftime(self.format(format))

    def yesterday_utc_string(
        self, format: Optional[str] = None, ms: bool = True, isuc: bool = False
    ) -> str:
        return self.datetime_to_string(
            self.yesterday_utc_datetime(ms=ms), self.format(format), isuc=isuc
        )

    def tomorrow_utc_string(
        self, format: Optional[str] = None, ms: bool = True, isuc: bool = False
    ) -> str:
        return self.datetime_to_string(
            self.tomorrow_utc_datetime(ms=ms), self.format(format), isuc=isuc
        )

    def yesterday_local_string(
        self,
        format: Optional[str] = None,
        ms: bool = True,
        timezone: Optional[str] = None,
        ...
        return self.datetime_to_string(
            self.yesterday_local_datetime(ms=ms, timezone=timezone),
            self.format(format),
            isuc=isuc,
        )

    def tomorrow_local_string(
        self,
        format: Optional[str] = None,
        ms: bool = True,
        timezone: Optional[str] = None,
        ...
        return self.datetime_to_string(
            self.tomorrow_local_datetime(ms=ms, timezone=timezone),
            self.format(format),
            isuc=isuc,
        )

    def several_days_ago_string(
        self,
        dt: Optional[datetime.datetime] = None,
        format: Optional[str] = None,
        utc: bool = True,
        ...
        return self.datetime_to_string(
            self.several_days_ago(dt=dt, utc=utc, ms=ms, timezone=timezone, days=days),
            self.format(format),
            isuc=isuc,
        )

    def several_days_coming_string(
        self,
        dt: Optional[datetime.datetime] = None,
        format: Optional[str] = None,
        utc: bool = True,
        ...
        return self.datetime_to_string(
            self.several_days_coming(
                dt=dt, utc=utc, ms=ms, timezone=timezone, days=days
            ),
            self.format(format),
            isuc=isuc,
        )

    def several_time_ago_string(
        self,
        dt: Optional[datetime.datetime] = None,
        format: Optional[str] = None,
        utc: bool = True,
    ...
    def several_time_coming_string(
        self,
        dt: Optional[datetime.datetime] = None,
        format: Optional[str] = None,
        utc: bool = True,
    ...
    def utc_string(
        ...
        final_dt = (
            self.utc_datetime(value=local_dt) if not final_dt and local_dt else final_dt
        )
    ...
    def local_string(
        ...
        final_dt = (
            self.__to_local_datetime(dt=utc_dt) if not final_dt and utc_dt else final_dt
        )
        if isinstance(final_dt, (str, bytes)):
    ...
    def utc_date_string(
        ...
        return self.utc_string(
            ...
            utc_dt=utc_dt,
            isuc=isuc,
            value_format=self.value_format(value or dt, value_format),
        )
    ...
    def datetime_to_timestamp(
        self, dt: Union[datetime.datetime, datetime.date], ms: bool = False
    ) -> int:
    ...
    def string_to_date(
        self, string: str, format: Optional[str] = None
    ) -> Optional[datetime.date]:
    ...
    def utc_string_to_utc_date(
        self, utc_string: str, format: Optional[str] = None
    ) -> Optional[datetime.date]:
    ...
    def utc_string_to_utc_datetime(
        self, utc_string: str, format: Optional[str] = None
    ) -> Optional[datetime.datetime]:
        format = self.value_format(utc_string, format)
        if not self.validate_string(utc_string, format):
            return None
        return (
            self.__to_utc_datetime(self.string_to_datetime(utc_string, format))
            + self.offset()
        )

    def utc_string_to_local_datetime(
        self, utc_string: str, format: Optional[str] = None
    ) -> Optional[datetime.datetime]:
        format = self.value_format(utc_string, format)
        if not self.validate_string(utc_string, format):
            return None
        return (
            self.__to_local_datetime(self.string_to_datetime(utc_string, format))
            + self.offset()
        )
    ...
    def string_to_timestamp(
        self, string: str, format: Optional[str] = None, ms: bool = False
    ) -> Optional[int]:
        return self.string_to_local_timestamp(string, format, ms=ms)

    def string_to_utc_timestamp(
        self, string: str, format: Optional[str] = None, ms: bool = False
    ) -> Optional[int]:
        format = self.format(format)
        if not self.validate_string(string, format):
        ...
        return self.datetime_to_timestamp(
            self.string_to_utc_datetime(string, format), ms=ms
        )

    def string_to_local_timestamp(
        self, string: str, format: Optional[str] = None, ms: bool = False
    ) -> Optional[int]:
        format = self.format(format)
        if not self.validate_string(string, format):
            return None
        return self.datetime_to_timestamp(
            self.string_to_local_datetime(string, format), ms=ms
        )
    ...
    def utc_timestamp_to_utc_datetime(self, stamp: int) -> datetime.datetime:
        ...
        return self.make_aware(
            self.timestamp_to_datetime(stamp), timezone=self.timezone("UTC")
        )
    ...
    def timestamp_delta(
        self, stamp1: int, stamp2: int, interval: Optional[int] = None
    ) -> Dict[str, Any]:
        delta = stamp1 - stamp2
        abs_delta = abs(delta)
    ...
    def datetime_delta(
        ...
        return self.timestamp_delta(
            self.datetime_to_timestamp(dt1), self.datetime_to_timestamp(dt2), interval
        )
    ...
    def string_delta(
        ...
        format = self.format(format)
        if (not self.validate_string(string1, format1 or format)) or (
            not self.validate_string(string2, format2 or format)
        ):
            return None
        return self.timestamp_delta(
            self.string_to_timestamp(string1, format1 or format),
            self.string_to_timestamp(string2, format2 or format),
            interval,
        )
    ...
    def timestamp_countdown(self, stamp: int, utc: bool = True) -> int:
        return abs(
            min((self.utc_timestamp() if utc else self.local_timestamp()) - stamp, 0)
        )

    def datetime_countdown(self, dt: datetime.datetime) -> int:
        return self.timestamp_countdown(
            self.datetime_to_timestamp(self.__to_utc_datetime(dt))
        )

    def string_countdown(
        self, string: str, format: Optional[str] = None
    ) -> Optional[int]:
        format = self.format(format)
        if not self.validate_string(string, format):
    ...
    def utc_datetime_midnight(
        self, utc_dt: Optional[datetime.datetime] = None
    ) -> datetime.datetime:
        return (self.__utc_datetime(utc_dt)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )

    def utc_seconds_since_midnight(
        self,
        utc_dt: Optional[datetime.datetime] = None,
        seconds_cast_func: Callable[[Any], T] = float,
    ) -> T:
        utc_dt = self.__utc_datetime(utc_dt)
        return seconds_cast_func(
            self.total_seconds(utc_dt - self.utc_datetime_midnight(utc_dt))
        )

    def local_datetime_midnight(
        self, local_dt: Optional[datetime.datetime] = None
    ) -> datetime.datetime:
        return (self.__local_datetime(local_dt)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
    ...
    def local_seconds_since_midnight(
        ...
        return seconds_cast_func(
            self.total_seconds(local_dt - self.local_datetime_midnight(local_dt))
        )

    def datetime_midnight(
        self, dt: Optional[datetime.datetime] = None, utc: bool = False
    ) -> datetime.datetime:
        return (
            self.utc_datetime_midnight(dt) if utc else self.local_datetime_midnight(dt)
        )
    ...
    def seconds_since_midnight(
        ...
        return seconds_cast_func(
            self.utc_seconds_since_midnight(dt)
            if utc
            else self.local_seconds_since_midnight(dt)
        )
    ...
    def make_aware(
        self, value: datetime.datetime, timezone: Optional[str] = None
    ) -> datetime.datetime:
    ...
    def is_past_time(
        ...
        if isinstance(value, (str, bytes)):
            utc_dt = (
                self.utc_string_to_utc_datetime(value, format=format)
                if utc
                else self.string_to_utc_datetime(value, format=format)
            )
            return utc_dt and utc_dt < base_dt

        if isinstance(value, int):
            stamp = self.datetime_to_timestamp(
                base_dt if utc else self.__to_local_datetime(base_dt), ms=True
            )
            return value < stamp
    ...
    def year(
        ...
        return self.__datetime(
            dt=self.several_time_coming(dt=dt, utc=utc, timezone=timezone, years=idx),
            utc=utc,
        ).year
    ...
    def month(
        ...
        return self.__datetime(
            dt=self.several_time_coming(dt=dt, utc=utc, timezone=timezone, months=idx),
            utc=utc,
        ).month
    ...
    def day(
        ...
        return self.__datetime(
            dt=self.several_time_coming(dt=dt, utc=utc, timezone=timezone, days=idx),
            utc=utc,
        ).day
    ...
    def days_of_month(
        ...
        return calendar.monthrange(
            year=(year or self.year(dt, idx=idx)),
            month=(month or self.month(dt, idx=idx)),
        )[-1]
    ...
    def total_seconds(self, td: datetime.timedelta, ms: bool = True) -> int:
        ...
        return (
            (td.days * 86400 + td.seconds) * self.SECOND_MICROSECOND + td.microseconds
    ...
    def date_range(
        ...
        return_type: str = "date",
        return_format: Optional[str] = None,
    ) -> Callable:
        if isinstance(start_date, str):
            start_date = self.string_to_date(
                start_date, start_date_format or format or self.DATE_FORMAT
            )
        if isinstance(end_date, str):
            end_date = self.string_to_date(
                end_date, end_date_format or format or self.DATE_FORMAT
            )
        if include_end:
            end_date = end_date + datetime.timedelta(1)
        if return_type in ["string", "str"]:
            for n in range(int((end_date - start_date).days)):
                yield self.datetime_to_string(
                    start_date + datetime.timedelta(n),
                    return_format or format or self.DATE_FORMAT,
                )
        else:
            for n in range(int((end_date - start_date).days)):
        ...
        if return_type in ["string", "str"]:
            for n in range(int(end_week - start_week) + 1):
                ...
                yield {
                    ...
                    "start": self.datetime_to_string(
                        current_week.monday(),
                        return_format or format or self.DATE_FORMAT,
                    ),
                    "end": self.datetime_to_string(
                        current_week.sunday(),
                        return_format or format or self.DATE_FORMAT,
                    ),
                }
    ...
    def between(
        self,
        value: TimeAnyT,
        start_value: TimeAnyT,
        end_value: TimeAnyT,
        timezone: Optional[str] = None,
        format: Optional[str] = None,
        start_format: Optional[str] = None,
        end_format: Optional[str] = None,
    ) -> bool:
        value = self.to_datetime(value, timezone=timezone, format=format, dttype="utc")
        start_value = self.to_datetime(
            start_value, timezone=timezone, format=format or start_format, dttype="utc"
        )
        end_value = self.to_datetime(
            end_value, timezone=timezone, format=format or end_format, dttype="utc"
        )
