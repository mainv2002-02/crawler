...
class AdapterInterface(MutableMapping, metaclass=ABCMeta):
    ...
    @classmethod
    def get_field_meta_from_class(
        cls, item_class: type, field_name: str
    ) -> MappingProxyType:
...
class AttrsAdapter(_MixinAttrsDataclassAdapter, AdapterInterface):
    ...
    def get_field_meta_from_class(
        ...
        try:
            ...
            raise KeyError(
                f"{item_class.__name__} does not support field: {field_name}"
            )
...
class DataclassAdapter(_MixinAttrsDataclassAdapter, AdapterInterface):
    def __init__(self, item: Any) -> None:
        ...
        # store a reference to the item's fields to avoid O(n) lookups and O(n^2) traversals
        self._fields_dict = {
            field.name: field for field in dataclasses.fields(self.item)
        }
...
class ItemAdapter(MutableMapping):
    ...
    def __init__(self, item: Any) -> None:
        for cls in self.ADAPTER_CLASSES:
            ...
            raise TypeError(
                f"No adapter found for objects of type: {type(item)} ({item})"
            )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<{self.__class__.__name__} for {self.item.__class__.__name__}({values})>"
        )
