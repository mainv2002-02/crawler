...
if (
    not hasattr(settings, "DJANGO_ADMIN_DISABLE_DELETE_SELECTED")
...
class AdvancedActionsModelAdmin(object):
    def get_action_choices(self, request, default_choices=BLANK_CHOICE_DASH):
        ...
        """
        actions_exclude = (
            self.actions_exclude if hasattr(self, "actions_exclude") else []
        )
...
class DeleteModelAdmin(object):
    ...
    override_delete_selected.short_description = _(
...
class ExportExcelModelAdmin(object):
    ...
    def export_excel(modeladmin, request, queryset):
        force_csv = (
            hasattr(settings, "DJANGO_EXCEL_RESPONSE")
            and settings.DJANGO_EXCEL_RESPONSE
        ) or (hasattr(modeladmin, "force_csv") and modeladmin.force_csv)
        return ExcelResponse(
            queryset,
            output_name=modeladmin.model._meta.verbose_name_plural,
            force_csv=force_csv,
        )

    export_excel.short_description = _(
...
class AdvancedExportExcelModelAdmin(object):
    ...
    def excel_item(modeladmin, query, field):
        ...
        return str(
            getattr(query, foo_field)()
            if hasattr(query, foo_field)
            else template_localtime(getattr(query, field))
        )
    ...
    def excel_data(modeladmin, request, query, model_fields, has_extra_excel_fields):
        ...
        return (
            excel_item + list(modeladmin.add_extra_excel_fields(request, query))
            if has_extra_excel_fields
            else excel_item
        )
    ...
    def advanced_export_excel(modeladmin, request, queryset):
        ...
        has_extra_excel_fields = hasattr(
            modeladmin, "extra_excel_fields"
        )  # Add by call add_extra_excel_fields

        model_fields = (
            list(modeladmin.excel_fields)
            if has_excel_fields
            else [f.name for f in modeladmin.model._meta.fields]
        )
        if has_excel_fields_exclude:
            model_fields = [
                field
                for field in model_fields
                if field not in set(modeladmin.excel_fields_exclude)
            ]

        excel_headers = (
            modeladmin.excel_headers
            if has_excel_headers
            else (
                model_fields + list(modeladmin.extra_excel_fields)
                if has_extra_excel_fields
                else model_fields
            )
        )
        excel_headers = (
            [
                (modeladmin.excel_headers_mapping.get(header) or header)
                for header in excel_headers
            ]
            if has_excel_headers_mapping
            else excel_headers
        )

        excel_data = [excel_headers]
        excel_data += [
            modeladmin.excel_data(request, query, model_fields, has_extra_excel_fields)
            for query in queryset
        ]
    ...
    advanced_export_excel.short_description = _(
        "Advanced Export selected %(verbose_name_plural)s as Excel"
    )
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        if obj:  # editing an existing object
            return tuple(
                ...
                - set(self.readonly_fields_exclude)
            )
